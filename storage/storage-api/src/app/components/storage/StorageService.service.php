    ...
    class StorageService {
        ...
        */
        public function makeDir($user, $path, $name) {
            ...
            if (!is_null($this->getChild($name, $parent))) {
                // A file or a directory already exists
                throw new FileAlreadyExistsException('File with name \'' . $name . '\' already exists in the path \'' . $path . '\'!');
            }
...
SQL;
        ...
        private function getDirectory($user, $path) {
            ...
            foreach ($pathExploded as $directory) {
                ...
                if (is_null($current)) {
                    throw new DirectoryNotFoundException('Some subdirectory of \''. $path . '\' does not exists!');
                }
            }
...
SELECT uuid
    FROM file
    WHERE user_uuid = :user AND file_name = '/' AND is_dir = FALSE
SQL;
...
SELECT F.uuid
    ...
    WHERE H.uuid_parent = :parent_uuid AND F.file_name = :file_name AND F.is_dir = TRUE
SQL;
            ...
            // Bind query parameters
            $stmt->bindParam(':parent_uuid', $parent, PDO::PARAM_STR, 36);
            $stmt->bindParam(':file_name', $file_name, PDO::PARAM_STR, 255);
